...
import ChatEntity from '../entity/entity-chat';
...
export default function ChatPage({ navigation, route }) {
  ...
  async function getMessages() {
    onValue(ref(db, `/messages/${route.params.item.id}`), (snapshot) => {
      try {
        setMessages([]);
  ...
  async function sendMessage() {
    ...
    await push(ref(db, `messages/${route.params.item.id}`), newMessage);
    setInputMessage('');
  }
